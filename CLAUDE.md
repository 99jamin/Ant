...
# 모델 협업 규칙 (지휘관-일꾼 패턴)
- **Opus 4.6 (The Architect):** 프로젝트 맥락 파악, 아키텍처 설계, 복잡한 로직 결정, 최종 코드 리뷰를 담당합니다.
- **Sonnet (The Worker):** 설계도가 확정된 후, 실제 파일 생성, 코드 타이핑, 단순 리팩토링, 주석 작성 등 실무를 서브 에이전트로 위임받아 수행합니다.
- **작업 흐름:** [Opus의 설계안 제안] → [사용자 승인] → [Sonnet 서브 에이전트로 구현 위임]